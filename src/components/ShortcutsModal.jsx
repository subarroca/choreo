...
export default function ShortcutsModal({ onClose }) {
  return (
    <>
      ...
      <div className="fixed inset-y-4 left-1/2 -translate-x-1/2 z-50 w-full max-w-lg bg-pane border border-line rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-rim shrink-0">
          <h2 className="text-sm font-semibold text-body">Dreceres de teclat</h2>
        ...
        <div className="overflow-y-auto px-5 py-4 space-y-6">
          {SECTIONS.map(section => (
            <div key={section.title}>
              <p className="text-[11px] font-semibold uppercase tracking-wider text-faint mb-3">{section.title}</p>
              <div className="space-y-1">
                {section.shortcuts.map(({ key, desc }) => (
                  <div key={key} className="flex items-center gap-3 py-1.5">
                    <kbd className="shrink-0 bg-fill border border-line rounded-md px-2 py-1 text-xs text-soft font-mono min-w-[90px] text-center">
                      {key}
                    </kbd>

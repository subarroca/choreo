...
export default function SortableMomentRow({ moment, index, showId, songId, onDelete, onCopy, micAssignments, positions, changedMembers, soloists, allMembers, gridRows, gridCols }) {
  ...
  return (
    <div ref={setNodeRef} style={style}
      className="flex items-center gap-2 pl-8 pr-3 py-2 border-b border-rim/50 bg-black/20 hover:bg-black/40 border-l-2 border-l-gray-700/60 group transition-colors">
      <button {...attributes} {...listeners}
        className="text-gray-700 hover:text-faint cursor-grab active:cursor-grabbing p-2 -ml-2 touch-none shrink-0">
        <GripVertical size={16} />
      </button>
      <span className="text-xs text-ghost w-5 text-center shrink-0 tabular-nums">{index + 1}</span>
      ...
      {activeMicCount > 0 && (
        <span className="flex items-center gap-0.5 text-xs text-faint shrink-0">
          <MicVocal size={10} className="text-ghost" />
          {activeMicCount}
        </span>
      )}
      {onCopy && (
        <button onClick={() => onCopy(moment)}
          className="text-gray-700 hover:text-cyan-400 p-2.5 rounded-lg hover:bg-fill transition-colors shrink-0"
          title="Copiar moment">
          <Copy size={14} />
        </button>
      )}

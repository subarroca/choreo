  ...
  return (
    <div ref={setNodeRef}
      ...
      <input value={label} onChange={e => onEdit(e.target.value)}
        className="flex-1 min-w-0 bg-fill border border-line rounded px-1.5 py-1.5 text-xs text-body focus:outline-none focus:border-cyan-300" />
      <input value={elevation ?? 0} onChange={e => onEditElevation(e.target.value)}
        type="number" min="0" max="300" step="5"
        className="w-12 bg-fill border border-line rounded px-1 py-1.5 text-xs text-soft focus:outline-none focus:border-cyan-300 tabular-nums"

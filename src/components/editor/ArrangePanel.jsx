      ...
      <div className="flex gap-1 mb-3">
        {['cols','rows'].map(ax => (
          <button key={ax} onClick={() => onSetArrangeAxis(ax)}
            className={`flex-1 py-2 rounded-lg text-xs border transition-colors ${arrangeAxis === ax ? 'border-violet-600 text-violet-300 bg-violet-900/30' : 'border-line text-muted hover:text-body'}`}>
        ...
        {ARRANGEMENT_PATTERNS.map(pat => {
          ...
          return (
            <button key={pat} onClick={() => onPick(pat)}
              className="flex flex-col items-center gap-1 p-1.5 rounded-lg border border-line hover:border-violet-600 hover:bg-violet-900/20 transition-colors min-h-[48px]">
              ...
              <span className="text-xs text-muted font-mono">{pat}</span>
            </button>
          )
        })}
      </div>
      <label className="flex items-center gap-2 text-xs text-muted cursor-pointer select-none py-1">
